...
public class ResponseFactoryImpl implements ResponseFactory {
	...
	public String getProductFamily(long productFamilyId) {
		...
		if (productFamily == null) {
			halt(ResponseFactory.httpNotFoundCode, "cannot find product family");
		}
	...
	public String getProduct(long productId) {
		...
		if (product == null) {
			halt(ResponseFactory.httpNotFoundCode, "cannot find product");
		}
	...
	public String getEventType(long eventTypeId) {
		...
		if (eventType == null) {
			halt(ResponseFactory.httpNotFoundCode, "Event type not found");
		}
	...
	public String getEvent(long eventId) {
		...
		if (event == null) {
			halt(ResponseFactory.httpNotFoundCode, "Event not found");
		}
	...
	public void createEventType(String requestBody) {
		try {
			...
			if (eventQuery == null || eventQuery.length() == 0) {
				halt(ResponseFactory.httpErrorCode, "no event query given in body");
			}

			if (jsonEventType == null) {
				halt(ResponseFactory.httpErrorCode, "no event type given in body");
			}

			EventType eventType = entityManager.createEventType(jsonEventType);

			if (eventType == null) {
				halt(ResponseFactory.httpErrorCode, "event type name already in use, or failed to parse event type");
			} else {
				...
				if (!eventPlatformRestEndpoint.getEventSubscriber().registerEventType(eventType)) {
					halt(ResponseFactory.httpErrorCode, "cannot register event type");
				}
		...
		try {
			...
			if (eventType == null) {
				...
				if (!eventType.isEditable()) {
					halt(ResponseFactory.httpForbiddenCode, "you must not edit this event type");
				}

				if (!eventPlatformRestEndpoint.getEventSubscriber().updateEventQuery(eventType, eventQuery)) {
					halt(ResponseFactory.httpErrorCode, "event platform did not accept the updated event query");
				}
	...
	public void updateStatusEventQuery(long productId, ProductState newState, String requestBody) {
		try {
			...
			if (product == null) {
				halt(ResponseFactory.httpNotFoundCode, "product not found");
			} else {

				if (product.getStatusUpdateQuery(newState) == null) {
					halt(ResponseFactory.httpErrorCode, "new state is not supported by this product");
				}
				...
				if (!eventPlatformRestEndpoint.getEventSubscriber().updateEventQuery(
					...
					halt(ResponseFactory.httpErrorCode, "event platform did not accept the updated status query");
				}
	...
	public String deleteEventType(long eventTypeId) {
		...
		if (eventType == null) {
			...
			if (!eventType.isDeletable()) {
				halt(ResponseFactory.httpForbiddenCode, "you must not delete this event type");
			}
